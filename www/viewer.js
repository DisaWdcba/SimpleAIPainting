...
/* ---------- History (StorageAdapter: SQLite via Bridge / IndexedDB fallback) ---------- */
let historyRenderSeq = 0;
let _historyCache = { items: [], total: 0, hasMore: false };

async function loadHistory(filter = {}) {
  const result = await StorageAdapter.queryHistory(filter);
  _historyCache = result;
  return result.items || [];
}

async function saveHistoryEntry(record, imageUrls) {
  try {
    await StorageAdapter.saveHistory(record, imageUrls);
  } catch (e) {
...
async function deleteHistoryEntry(id) {
  try {
    await StorageAdapter.deleteHistory(id);
  } catch (e) {
...
async function deleteAllHistory() {
  ...
  try {
    await StorageAdapter.deleteAllHistory();
  } catch (e) {
    console.error('[History] 清空失败:', e);
  }
  _historyCache = { items: [], total: 0, hasMore: false };
  void renderHistoryDrawer();
}
...
async function renderHistoryDrawer() {
  ...
  let items;
  if (StorageAdapter.isNative) {
    const result = await loadHistory(getHistoryFilters());
    if (seq !== historyRenderSeq) return;
    items = result;
    $('history-count').textContent = `${_historyCache.total} 条`;
  } else {
    const entries = await loadHistory();
    if (seq !== historyRenderSeq) return;
    const filtered = filterHistoryEntries(entries);
    items = filtered;
    $('history-count').textContent = `${filtered.length} / ${_historyCache.total || entries.length} 条`;
  }
  ...
  list.innerHTML = items.map(e => {
    const imageCount = e.imageCount || (Array.isArray(e.hits) ? e.hits.length : 0);
    ...
    return `<div class="hist-entry bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-3 hover:border-brand-400 dark:hover:border-brand-500 cursor-pointer transition-colors group" data-id="${e.id}">
      <div class="flex gap-3">
        ${thumb ? `<div class="w-14 h-14 shrink-0 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-600 bg-slate-100 dark:bg-slate-800"><img src="${thumb}" class="w-full h-full object-cover checkerboard" alt="" loading="lazy" /></div>` : `<div class="w-14 h-14 shrink-0 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 text-lg">${statusIcon}</div>`}
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-2 mt-1.5">
            ...
            ${imageCount > 0 ? `<span class="text-[10px] text-slate-400 dark:text-slate-500">${imageCount} 张</span>` : ''}
...
function getHistoryThumbnail(entry) {
  if (entry.thumbUrl) {
    const src = safeUrl(entry.thumbUrl);
    if (src) return src;
  }
  if (entry.fullUrl) {
    const src = safeUrl(entry.fullUrl);
    if (src) return src;
  }
  if (entry.hits && entry.hits.length > 0) {
    const hit = entry.hits[0];
    const src = safeUrl(hit.dataUrl || hit.thumbnailUrl || hit.localUrl || hit.url);
    if (src) return src;
  }
  return null;
}
...
async function showHistoryDetail(id) {
  ...
  try {
    if (StorageAdapter.isNative) {
      entry = await StorageAdapter.getHistoryById(id);
    } else {
      const entries = await loadHistory();
      entry = entries.find(e => e.id === id);
    }
  } catch (e) {
    console.error('[History] 获取详情失败:', e);
    return;
  }
  if (!entry) return;
  ...
  if (hits.length > 0) {
    imagesHtml = `<div class="space-y-3">
      ...
      ${hits.map((hit, i) => {
        const src = safeUrl(hit.localUrl || hit.thumbnailUrl || hit.dataUrl || hit.url);
        if (!src) return '';
...
async function addHistoryEntry(version, threadReq) {
  ...
  const imageUrls = version.hits ? version.hits.map(h => h.dataUrl || h.url || '') : [];

  await saveHistoryEntry(record, imageUrls);
  void renderHistoryDrawer();
}
